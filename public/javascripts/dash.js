...
var dash = {
  ...
  wsMessageHandler: function (messageEvent) {
    ...
    if (message.deviceId === dash.deviceId) {
      if (message.dataType === 'realtimeUsage') {
        dash.refreshRealtimeDisplay(message);
      }
  ...
  addLogEntry: function (logEntry, updateChart) {
    ...
    dash.usageLogChart && dash.usageLogChart.data.labels.push(moment(logEntry.ts, 'x').format("MMM Do HH:mm"));
    dash.usageLogChart && dash.usageLogChart.data.datasets.forEach(function (dataset) {
      dataset.data.push({
        x: logEntry.ts,
        y: logEntry.pw
      });
  ...
  refreshRealtimeDisplay: function (message) {
    const realtime = message.data;
